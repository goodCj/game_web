...
const Gohome = () => {
  const { cam = null, home = null, more = null, clean = null, scroll = null, sec = 10, first=null} = qs.parse(window.location.search)
  console.log(cam)
  return (
    <FloatingBubble
      axis="xy"
    ...
    >
      <a onClick={() => {
        ...
        }} className="homeBtn" href={`https://${window.location.hostname.split('.').slice(-2).join('.')}?cam=${cam}&home=${home}&more=${more}&clean=${clean}&sec=${sec}&first=${first}`}>
      <Space>
          <svg
            t="1677682941120"
            class="icon"
